...
with st.sidebar:
    choice = option_menu("Main Menu", menu, menu_icon="cast", default_index=0)
...
if choice == 'Login':
    ...
    if st.button('Login'):
        ...
        st.success('Loged In Sucessfully')
    ...
    if 'logged_in' in st.session_state and st.session_state['logged_in'] and st.session_state['role'] == 'admin':
        ...
        cols = st.columns(3)
        with cols[0]:
            ui.metric_card(title="Total grievances", content=total, description="+20.1% from last month", key="card1")
        with cols[1]:
            ui.metric_card(title="Pending grievances", content=pending, description="+20.1% from last month", key="card2")
        with cols[2]:
            ui.metric_card(title="Completed grievances", content=completed, description="+20.1% from last month", key="card3")
